...
//        if (detectedCodec) {
//            // 使用检测到的编码转换
//            utf8Data = detectedCodec->toUnicode(data).toUtf8();
//        } else {
//            // 3. 回退方案：使用Latin1（无损转换）
//            qWarning() << "Failed to detect encoding for data, using Latin1 fallback. Size:" << data.size();
//            utf8Data = QString::fromLatin1(data).toUtf8();
//        }
//    }

//    // === 数据包构造 ===
//    // 检查长度是否超过quint16范围（防御性检查）
//    if (utf8Data.size() > std::numeric_limits<quint16>::max()) {
//        qCritical() << "Data size exceeds maximum allowed:" << utf8Data.size();
//        return; // 或截断数据，根据业务需求决定
//    }

//    quint16 len = static_cast<quint16>(utf8Data.size());
...
//    // 添加数据（防御性检查空数据）
//    if (!utf8Data.isEmpty()) {
//        block.append(utf8Data);
//    } else {
//        qWarning() << "Sending empty data packet for ID:" << id;
//    }
...
//            qWarning() << "Partial data sent. Expected:" << block.size()
//                       << "Actual:" << bytesWritten;
//        }
//    } else {
//        qCritical() << "Attempted to send data while socket not connected. State:"
//                   << _socket.state();
//    }
//}
