...
bool RedisMgr::Set(const std::string& key, const std::string& value)
{
	...
	// 返回NULL表示失败
	if (reply == NULL) {
		std::cerr << "Execut command [ SET " << key << "  " << value << " ] failure ! " << std::endl;
		freeReplyObject(reply);
		return false;
	}
	//如果执行失败则释放连接
...
bool RedisMgr::LPop(const std::string& key, std::string& value)
{
	...
	if (reply == nullptr || reply->type == REDIS_REPLY_NIL) {
		std::cerr << "Execut command [ LPOP " << key << " ] failure ! " << std::endl;
		freeReplyObject(reply);
		return false;
	}
	value = reply->str;
...
bool RedisMgr::RPop(const std::string& key, std::string& value)
{
	...
	if (reply == nullptr || reply->type == REDIS_REPLY_NIL) {
		std::cerr << "Execut command [ RPOP " << key << " ] failure ! " << std::endl;
		freeReplyObject(reply);
		return false;
	}
	value = reply->str;
...
bool RedisMgr::HSet(const std::string& key, const std::string& hkey, const std::string& value)
{
	...
	if (reply == nullptr || reply->type != REDIS_REPLY_INTEGER) {
		std::cerr << "Execut command [ HSet " << key << "  " << hkey << "  " << value << " ] failure ! " << std::endl;
		freeReplyObject(reply);
		return false;
	}
...
RedisConPool::RedisConPool(size_t poolSize, const char* host, int port, const char* passwd)
